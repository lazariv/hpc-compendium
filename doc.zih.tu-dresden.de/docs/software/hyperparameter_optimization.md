...
1. **Graphical approach:**
    ...
    After creating the plot OmniOpt suggests to open the html file directly in a web browser on the ZIH system.
    Therefore, it is neccessary to login via ssh with the option -X (X11 forwarding), e.g. ```ssh -X taurus.hrsk.tu-dresden.de```.
    Nevertheless, because of latency using x11 forwarding it is recommended to download the html file and explore the parallel plot on the local machine.
